...
#' timeseries1 <- simDrift(startingabs=rep(J1/nsp,nsp),
#'   ts=ages[1:(length(ages)/2 + 1)],ss=ss)$simulation #first half
#' timeseries2 <- simDrift(startingabs=timeseries1[1,]*J2/ss,
#'   ts=ages[1:(length(ages)/2 + 1)],ss=ss)$simulation #second half
...
#'   ages[length(ages)],fit$CI_shiftage[2],fit$CI_shiftage[2],ages[1]),
#'   border=NA,col=rgb(0.7, 0.7, 0.7, 0.5))
...
fitJshift <- function(occs,ages,liksurf=FALSE,sampled=TRUE,generationtime=1,
  ...
  for(i in 2:(length(ages)-1)){ #consider all shift ages except first and last ages
    ...
    bestfits <- append(bestfits,list(list("loglik"=fit1$loglik+fit2$loglik,"J1"=fit1$J,"J2"=fit2$J,
                                          "shiftage"=ages[i])))}
  #last age
  bestfits <- append(bestfits,list(list("loglik"=noshift$loglik,"J1"=noshift$J,"J2"=NA,
                                        "shiftage"=ages[length(ages)])))
  #ML estimate
  ...
  if(CI){
    ...
    out$CI_shiftage <- c(ages[utils::head(shiftage_indices,1)],
                         ages[utils::tail(shiftage_indices,1)])}
  return(out)}
